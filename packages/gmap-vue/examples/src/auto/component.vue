  ...
  <div class="component">
    ...
    <transition name="expand">
      <div v-if="showDocumentation">
        ...
        <h2>
          Properties mapped
          <template v-if="!component.is3rdParty">from Google Maps API</template>
        </h2>
        <CiteRef v-if="!component.is3rdParty" />
        <ul>
          <li
            v-for="[key, value] in sort(Object.entries(component.mappedProps))"
            :key="key"
          >
            <code
              ><b>{{ key }}</b></code
            ...
            <ul>
              ...
              <li v-if="value.twoWay">
                A <code>{{ key }}_changed</code> event will be emitted if the
                property is modified by the user interacting with the UI.
              </li>
            </ul>

            <div v-if="value.description" v-html="value.description"></div>
          </li>
        </ul>
        ...
        <template v-if="component.props">
          ...
          <ul>
            <li
              v-for="[key, value] in sort(Object.entries(component.props))"
              :key="key"
            >
              <code
                ><b>{{ key }}</b></code
        ...
        <template v-if="!component.isIrregular">
          ...
          <ul>
            <li>
              <code>${{ component.name }}Object</code> &mdash; The underlying
              instance of a
              <code>{{ capitalize(component.name) }}</code> object.
            </li>
            <li>
              <code>${{ component.name }}Promise</code> &mdash; A promise that
              resolves to the underlying
              <template v-if="!component.is3rdParty">Google Maps API</template>
              instance when the instance has been created.
            </li>
          </ul>
        </template>
        ...
        <template v-if="component.inject">
          ...
          <ul>
            <li
              v-for="[key] in sort(Object.entries(component.inject))"
              :key="key"
            >
