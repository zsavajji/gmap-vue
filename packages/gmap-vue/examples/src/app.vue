...
<template>
  <div class="app-panel">
    <div class="settings-panel">
      ...
      Map zoom: <input type="number" v-model.number.lazy="zoom" /> <br />
      Dragged {{ drag }} times <br />
      Left clicked {{ mapClickedCount }} times <br />
      Map type:
      ...
      <br />
      Enable scrollwheel zooming on the map:
      <input type="checkbox" v-model="scrollwheel" />
      <br />
      <button @click="addMarker">Add a new Marker</button> (or right click on
      the map :) )
      <h1>Clusters</h1>
      enabled: <input type="checkbox" v-model="clustering" />
      <br />
      Grid size: <input type="number" v-model.number="gridSize" />
      <br />
      <h1>Polyline</h1>
      Editable: <input type="checkbox" v-model="pleditable" />
      <button @click="resetPlPath">Reset path</button>
      <br />
      Visible: <input type="checkbox" number v-model="plvisible" />
      <br />
      <h1>Polygon</h1>
      Visible: <input type="checkbox" v-model="pgvisible" /> <br />
      <button @click="pgPath = opgPath">Reset Polygon to pentagon</button><br />
      <button @click="pgPath = originalPlPath">
        Reset Polygon to a simple polygon</button
      ><br />
      Path: {{ pgPath | json }}
      <br />
      <h1>Circle</h1>
      Visible: <input type="checkbox" v-model="displayCircle" /><br />
      {{ circleBounds | json }}
      <br />
      <h1>Rectangle</h1>
      Visible: <input type="checkbox" v-model="displayRectangle" /><br />
      ...
      modal 2: <input type="checkbox" number v-model="ifw2" />
      <input type="text" v-model="ifw2text" />
      <h1>Markers</h1>
      Display only markers with even ID (to test filters)
      <input type="checkbox" number v-model="markersEven" /><br />
      <table>
        <tr>
          <th>lat</th>
          <th>lng</th>
    ...
    >
      ...
      <gmap-info-window :position="reportedCenter" :opened="ifw">
        To show you the bindings are working I will stay on the center of the
        screen whatever you do :)
        <br />
        To show you that even my content is bound to vue here is the number of
        time you clicked on the map
        <b>{{ mapClickedCount }}</b>
      </gmap-info-window>
...
export default {
  ...
  computed: {
    activeMarkers() {
      if (this.markersEven) {
        return this.markers.filter((v, k) => k % 2 === 0);
      }
      return this.markers;
    },
    enabledMarkers() {
      if (!Array.isArray(this.activeMarkers)) {
        throw new Error('activeMarkers is not an array');
      }

      return this.activeMarkers.filter((marker) => marker.enabled);
    },
    mapStyles() {
  ...
  methods: {
    // eslint-disable-next-line no-unused-vars -- with informative puroposes
    updateMapCenter(which, value) {
      this.center = _.clone(this.reportedCenter);
    },
    mapClicked(mouseArgs) {
      console.log('map clicked', mouseArgs); // eslint-disable-line no-console -- informative purposes
    },
    ...
    // eslint-disable-next-line no-unused-vars -- informative purposes
