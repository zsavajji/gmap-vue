import assert from 'assert';
import 'core-js/stable';
import fs from 'fs';
import path from 'path';
import 'regenerator-runtime/runtime';
import { getPage, loadFile } from './test-setup/test-common';

const Lab = require('@hapi/lab');

const lab = Lab.script();

exports.lab = lab;

lab.experiment('Basic tests', { timeout: 15000 }, () => {
  let page = null;

  async function loadPage() {
    return loadFile(page, './test-pages/test-plain-map.html', {
      waitUntil: 'domcontentloaded',
    });
  }

  async function mountVue() {
    return page.evaluateHandle(
      () =>
        new Promise((resolve) => {
          // eslint-disable-next-line no-undef -- this should be analyzed later
          new Vue({
            created() {
              resolve(this);
            },
          }).$mount('#test1');
        })
    );
  }

  lab.before(
    { timeout: 15000 },
    getPage((p) => {
      page = p;
    })
  );

  lab.test('Maps API is loaded', async () => {
    await loadPage();
    const vue = await mountVue();

    assert(
      await page.evaluate(
        (vue2) =>
          vue2.$refs.map.$mapPromise.then(
            () => vue2.$refs.map.$mapObject instanceof google.maps.Map
          ),
        vue
      ),
      '$mapPromise is defined'
    );

    assert(
      await page.evaluate(
        (vue2) =>
          vue2.$refs.map.$mapObject
            .getDiv()
            .parentNode.classList.contains('map-container'),
        vue
      ),
      'Parent of $mapObject.div is a .map-container'
    );
  });

  lab.test('Panning of map works', { timeout: 30000 }, async () => {
    await loadPage();
    const vue = await mountVue();

    const [top, right, bottom, left] = await page.evaluate(() => {
      const el = document.querySelector('.map-container');
      const containerTop = el.offsetTop;
      const containerRight = el.offsetLeft + el.offsetWidth;
      const containerBottom = el.offsetTop + el.offsetHeight;
      const containerLeft = el.offsetLeft;

      return [containerTop, containerRight, containerBottom, containerLeft];
    });

    // Wait for map to load first...
    await page.evaluate(
      (vue2) =>
        vue2.$refs.map.$mapPromise.then(
          () => new Promise((resolve) => setTimeout(resolve, 500))
        ),
      vue
    );

    // Then try to pan the page
    await page.mouse.move(right - 4, top + 4);
    await page.mouse.down();
    await page.mouse.move(left + 4, bottom - 4, { steps: 20 });
    await new Promise((resolve) => setTimeout(resolve, 100));
    await page.mouse.up();

    const { lat, lng } = await page.evaluate((vue2) => {
      const c = vue2.$refs.map.$mapObject.getCenter();
      return { lat: c.lat(), lng: c.lng() };
    }, vue);
    assert(lat > 1.45, 'Lat greater than 1.45');
    assert(lng > 103.9, 'Lng greater than 103.9');
  });

  lab.test('Lodash library is not bloating up the library', async () => {
    const libraryOutput = fs.readFileSync(
      path.join(__dirname, '..', 'dist', 'gmap-vue.js'),
      'utf-8'
    );

    if (/Lodash <http(.*)>/.test(libraryOutput)) {
      assert(
        false,
        'Lodash found! This is bad because you are bloating up the library'
      );
    }
  });
});
