...
lab.experiment('On-demand API loading', { timeout: 15000 }, () => {
  let page = null;
  ...
  lab.test('Maps API is loaded only on demand', async () => {
    await loadPage();
    ...
    assert(
      await page.evaluate((vue2) => {
        return Promise.resolve(null)
          ...
          .then(() => vue2.$refs.gmap.$mapPromise.then(() => !!window.google))
          .then((isGoogleLoaded) => {
            const allScriptElements = Array.prototype.slice.call(
              document.getElementsByTagName('SCRIPT'),
              0
