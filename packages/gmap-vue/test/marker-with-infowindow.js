...
lab.experiment('Marker / Infowindow tests', { timeout: 15000 }, () => {
  let page = null;
  ...
  lab.test('Clicking the marker triggers the infowindow', async () => {
    await loadPage();

    await page.evaluate(() => {
      // Ensure that the map has been created
      return window.theVue.$refs.map.$mapPromise.then(() => {
